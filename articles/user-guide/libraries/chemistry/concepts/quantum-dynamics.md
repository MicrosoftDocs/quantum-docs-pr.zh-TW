...
當我們透過將一般傳統 Hamiltonian 轉換成配量語言來建立 Hamiltonian 運算子之後，我們就可以透過 \begin{align} \frac{d}{dt} \hat{f} = \ partial_t \hat{f} + [\hat{f}，\hat{H}]，\end{align} （其中 $ [f，H] = fH-Hf $ 稱為 commutator）來表達任意量子機械數量（也就是配量機械運算子） $ \hat{f} （t） $ 的動態。
