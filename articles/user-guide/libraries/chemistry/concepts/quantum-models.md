...
\begin{equation} H \_ {i，j} = \int \_ {-\infty} ^ \infty\int \_ {-\infty} ^ \infty \psi ^ { \* } \_ i (x \_ 1) \hat{H} \psi \_ j (x \_ 2) \dd ^ 3 到 \_ 1 \dd ^ 3 倍 \_ 2. \ 標籤 {eq： discreteHam} \end{equation}
...
可能是最簡單的這類基礎集是 Slater 類型的 Orbitals (停止) 這是 (orthogonalized 的 Schrödinger 方程式) 方案 (也就是 eigenfunctions 類型的原子) 的 \hat{H}。
...
也就是說，如果 $ \psi (x_1，x_2) $ 是兩個 electrons 的聯合配量狀態的 wave 函式，則必須將 $ $ \psi (x_1，x_2) =-\psi (x_2，x_1) 。
$ $ Pauli 排除原則，這會禁止兩個 electrons 處於相同的配量狀態，fascinatingly，這項法則的直接結果就是 intuited 的事實，就是如果我們交換兩個位于相同位置 $ \psi (的 electrons x_1，x_1) \mapsto \psi (x_1，x_1) \ne-\psi (x_1，x_1) $，除非 $ \psi (x_1，x_1) = $0。
