...
雖然第二個量化 Hamiltonians 是以 $a ^ \dagger $ (建立) 和 $a $ (annihilation) 的角度來表示，但這些作業並不是量子電腦中的基本作業。
...
Wigner 標記法是直接衍生的。
回想一下，state $ \ket {0} _j $ 表示微調 orbital $j $ 是空的，而 $ \ket {1} _j $ 則表示它已被佔用。
這表示 qubits 可以自然地儲存指定微調 orbital 的職業。
接著，$a ^ \ dagger_j \ket {0} _j = \ket {1} _j $ and $a ^ \ dagger_j \ket {1} _j = $0。
您很容易就能確認 \begin{align} a ^ \ dagger_j &= \begin{bmatrix}0 & 0 \\ \ 1 &0 \end{bmatrix} = \frac{X_j-iY_j} {2} ，\nonumber \\ \\ a_j &= \begin{bmatrix}0 & 1 \\ \ 0 &0 \end{bmatrix} = \frac{X_j + iY_j} {2} ，\end{align}，其中 $X _j $ 和 $Y _j $ 運算子是 Pauli $X $。

>[!NOTE]
> 在 Q# $ \ket {0} $ state 中，代表 $Z $ 運算子的 + 1 eigenstate。 在物理 $ \ket $ 的某些區域中， {0} 代表低能源地面狀態，因此為 $Z $ 運算子的-1 eigenstate。 因此，某些公式可能會與熱門的文獻不同。
...
幸好，$Z $ 運算子的字串 (稱為 Wigner 字串，) 取消後再進行這項替代。
在執行此 (並重新叫用該 $X _jY_j = iZ_j $) 之後，我們 \begin{equation} n_j = a ^ \ dagger_j a_j = \frac{ (1-Z_j) } {2} 。
\end{equation}
...
在此表單中建立 Hamiltonians 之後，我們可以使用一系列的量子模擬演算法，將 $e ^ {-iHt} $ 所產生的 dynamics 編譯成一系列基本閘道， (在某些使用者可定義的容錯能力) 中。
