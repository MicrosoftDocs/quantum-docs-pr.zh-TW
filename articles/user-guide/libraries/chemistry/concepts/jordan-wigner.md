...
>[!NOTE]
> 在 Q # 中，$ \ket {0} $ state 代表 $Z $ 運算子的 + 1 eigenstate。 在物理 $ \ket $ 的某些區域中， {0} 代表低能源地面狀態，因此為 $Z $ 運算子的-1 eigenstate。 因此，某些公式可能會與熱門的文獻不同。
...
在執行此程式（並重新叫用 $X _jY_j = iZ_j $）之後，我們會 \begin{equation} n_j = a ^ \ dagger_j a_j = \frac{（1-Z_j）} {2} 。
\end{equation}
