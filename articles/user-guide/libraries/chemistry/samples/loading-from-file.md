...
Broombridge 架構也包含要準備之初始狀態的建議。 `"|G⟩"` `"|E1⟩"` 檢查檔案可能會顯示這些狀態的標籤，例如或。 為了準備這些初始狀態， `qSharpData` 會取得與[上一節](xref:microsoft.quantum.chemistry.examples.energyestimate)相似的 Q # 配量演算法所使用的，但會有一個額外的參數選取所需的初始狀態。 例如，
```csharp
