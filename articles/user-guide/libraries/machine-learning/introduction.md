...
假設 learnable 量子電路 $U （\theta） $，其中 $ \theta $ 是參數的向量，並以 $M $ 來表示最後的測量，正確標籤推斷的平均可能性為 $ $ \begin{align} \mathcal{L} （\theta） = \frac {1} {| \mathcal{D} |} \left （\ sum_ {（x，y_1） \In\mathcal{D}} P （M = y_1 |U （\theta） x） + \ sum_ {（x，y_2） \in\mathcal{D}} P （M = y_2 |U （\theta） x） \right） \end{align} $ $，其中 $P （M = y | z） $ 是測量 $y $ in 量子 state $z $ 的機率。
...
如果針對每個 RULE1 $x $ 所推斷的標籤實際上與 $y $ 不同，則定型案例 $ （x，y） \in \mathcal{D} $ 會視為 $b*分類誤判*。 Misclassifications 的整體數目是指定偏差 $b $ 的分類器*定型分數*。 *最佳*的分類偏差 $b $ 可將定型分數降至最低。 假設預先電腦率估計 $ \{ P （M = y_2 |，這很容易看出。U （\theta） x） |（x，*） \in\mathcal{D} \} $，二進位搜尋可以在 interval $ （-0.5，+ 0.5） $ 中找到最佳的分類偏差，最多可達 $ \ log_2 （| \mathcal{D} |）$ 個步驟。
