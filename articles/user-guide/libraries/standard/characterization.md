...
`bitsPrecision`輸入對而言是唯一的 `RobustPhaseEstimation` ，而 `oracle` 和 `eigenstate` 則是常見的。
因此，在**H2Sample**中，作業可以接受具有表單輸入的反復階段估計演算法， `(DiscreteOracle, Qubit[]) => Unit` 以允許使用者指定任意階段估計演算法：
