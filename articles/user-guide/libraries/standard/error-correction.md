...
例如，如果我們測量 $Z _0 $， {0} 則在沒有錯誤的情況下，我們會針對 $ \ket{\overline} $ 和 $ \ket{\overline} $ 取得不同的結果 {1} ，因此會折迭編碼的狀態。
...
請注意，$Z _0 Z_1 \ket {000} = \ket {000} $，而該 $Z _0 Z_1 \ket {111} = \ket {111} $，所以我們結論，這項測量會對這兩個編碼狀態執行相同的動作。
...
> [!NOTE]
> 上述的位翻轉程式碼只能針對單一的位翻轉錯誤進行更正;也就是 `X` 在單一 qubit 上運作的作業。
> 套用 `X` 至一個以上的 qubit 會在復原之後，將 $ \ket{\overline {0} } $ 對應到 $ \ket{\overline {1} } $。
...
- <xref:microsoft.quantum.errorcorrection.encodeop>`= ((Qubit[], Qubit[]) => LogicalRegister)`：表示作業會使用 qubits 來代表資料和全新的 ancilla qubits，以產生錯誤更正程式碼的程式碼區塊。
- <xref:microsoft.quantum.errorcorrection.decodeop>`= (LogicalRegister => (Qubit[], Qubit[]))`：表示會 decompose 程式碼區塊的作業，將錯誤修正為數據 qubits，並使用 ancilla qubits 來代表症狀資訊。
- <xref:microsoft.quantum.errorcorrection.syndromemeasop>`= (LogicalRegister => Syndrome)`：表示應該用來從程式碼區塊中解壓縮症狀資訊的作業，而不會干擾受程式碼保護的狀態。

最後，canon 會提供 <xref:microsoft.quantum.errorcorrection.qecc> 類型來收集定義配量錯誤更正程式碼所需的其他類型。 與每個穩定配量代碼相關聯的程式碼長度 $n $、邏輯 qubits 的 $k $，以及 $d $ 的最小距離，通常會以標記法⟦ $n $，$k $，$d $ ⟧中，以方便群組在一起。 例如，函式會 <xref:microsoft.quantum.errorcorrection.bitflipcode> 定義⟦3，1，1⟧位翻轉程式碼：
...
請注意， `QECC` 類型不*not*包含復原功能。
這可讓我們變更用於更正錯誤的復原功能，而不需變更程式碼本身的定義;這項功能特別適用于將特性度量的意見反應納入復原所假設的模型中。

以這種方式定義程式碼之後，我們可以使用作業 <xref:microsoft.quantum.errorcorrection.recover> 從錯誤中復原：
