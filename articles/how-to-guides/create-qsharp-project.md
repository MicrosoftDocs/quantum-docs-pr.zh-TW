...
Q # 專案包含包含量子代碼的 Q # 檔案，以及執行配量程式的主機程式。 您可以在 .NET 語言（例如C#）或 Python 中撰寫主機程式。 您也可以使用 IQ # kernel 在 Jupyter 筆記本中執行 Q # 程式碼。
...
1. 建立名為 `Operation.qs`的 Q # 檔案，並將您的 Q # 程式碼加入其中。 例如：
...
1. 建立名為 `host.py` 的 python 主機檔案，以呼叫您的 Q # 作業。 例如：
...
1. Jupyter 頁面會出現在瀏覽器中。 在 檔案 索引卷**標上，** 選取 **新增** > **q #** ，以使用 Q # 核心來建立 Jupyter 筆記本。 將下列程式碼新增至第一個筆記本資料格：
