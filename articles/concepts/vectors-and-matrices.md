...
對向量和矩陣的熟悉程度，是瞭解量子運算的關鍵。 我們提供以下的簡短簡介，並且有興趣的讀者建議您閱讀線性代數的標準參考，例如 *Strang、g. (1993) 。線性代數的簡介 (Vol. 3) 。Wellesley，MA： Wellesley-康橋按* 或線上參考，例如 [線性代數](http://joshua.smcvt.edu/linearalgebra/)。
...
大小[*matrix*](https://en.wikipedia.org/wiki/Matrix_(mathematics))為 $ m n 的 \times 矩陣 $ 是 $ mn 複數的集合 $ ，以 m 個 $ 資料 $ 列和 n 個數據行排列， $ $ 如下所示：
...
$ $ [*Hermitian*](https://en.wikipedia.org/wiki/Hermitian_matrix)如果 $ m m ^，則會將矩陣 m 視為 Hermitian = \dagger $ 。
