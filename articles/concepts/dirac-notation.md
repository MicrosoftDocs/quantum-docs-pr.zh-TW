...
作者： QuantumWriter uid： dirac ms. 作者： v-benbra ms. date： 12/11/2017 ms. 主題：概念無 loc：
- "Q#"
- "$$v"
- "$$"
- "$$"
...
在這個標記法中 $ \ket { } $ ，0不需要參考單一量子位狀態，而是儲存二進位編碼為0的量子位 $ $ 暫存器。  這兩種標記法之間的差異通常會從內容中清楚明瞭。  這個慣例有助於簡化第一個可利用下列方式撰寫的範例：
