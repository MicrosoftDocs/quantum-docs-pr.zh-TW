...
既然我們已經知道如何呈現 qubit，我們可以藉由討論[*測量*](https://en.wikipedia.org/wiki/Measurement_in_quantum_mechanics)概念來取得這些狀態所代表的部分直覺。 量測會對應到 qubit 上「尋找」的非正式想法，這會立即折迭量子狀態為兩個傳統狀態 $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $ 或 $ \begin{bmatrix} 0 \\ \\ 1 \end{bmatrix} $ 的其中一個。 測量配量狀態向量 $ \begin{bmatrix} \Alpha \Beta \end{bmatrix} $ 所指定的 qubit 時 \\ \\ ，我們會取得結果 $0 $，其中機率為 $ | \Alpha | ^ 2 $，而結果 $1 $ 的機率為 $ | \Beta | ^ 2 $。 在結果 $0 $ 中，qubit 的新狀態為 $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $; 結果 $1 $ 其狀態為 $ \begin{bmatrix} 0 \\ \\ 1 \end{bmatrix} $。 請注意，由於正規化條件 $ | \Alpha | ^ 2 + | \Beta | ^ 2 = $1，這些機率會加總為 $1 $。

測量的屬性也表示量子狀態向量的整體正負號無關。 否定向量相當於 $ \Alpha \rightarrow-\Alpha $ 和 $ \Beta \rightarrow-\Beta $。 因為測量 $0 $ 和 $1 $ 的機率取決於詞彙的大小平方，所以插入這類符號並不會改變機率。 這類階段通常稱為[ `` *全域階段*' '](https://en.wikipedia.org/wiki/Phase_factor) ，而更普遍的形式可以是 $e ^ {i \phi} $，而不只是 $ \pm $1。

測量的最後一個重要屬性是，它不一定會損毀所有的量子狀態向量。 如果我們以 state $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $ 的 qubit 開始，其對應于傳統狀態 $0 $，測量此狀態一律會產生結果 $0 $，並讓配量狀態維持不變。 就這一點而言，如果我們只有傳統位（也就是 $ \begin{bmatrix}1 \\ \\ 0 \end{bmatrix} $ 或 $ \begin{bmatrix}0 \\ \\ 1 \end{bmatrix} $ 的 qubits），則測量不會損毀系統。 這表示我們可以在量子電腦上複寫傳統資料，並將它操作，就像在傳統電腦上一樣。 不過，若要一次儲存這兩個狀態的資訊，您可以將配量運算提升至傳統方式，並進一步 robs 量子電腦不加上複製量子資料的能力，另請參閱[不復制定理](https://en.wikipedia.org/wiki/No-cloning_theorem)。
...
雖然這是一個簡單的範例（因為作業 <xref:microsoft.quantum.intrinsic.h> 是自我 adjoint 的），您可以看到這對更複雜的 qubit 作業變得很有用。
...
換句話說，如果有任何單一轉換可以從這個集合中的閘道乘積來撰寫，則一組閘道就是通用閘道集。 針對任何指定的錯誤系結，我們需要有閘道集的閘道 $G _ {1} 、G_ {2} 、\ldots、G_N $

$ $ G_N G_ {N-1} \cdots G_2 G_1 \approx U. $ $

請注意，由於矩陣乘法的慣例是在此順序中的第一個閘道運算的正向右乘以，$G _N $，實際上是最後一個套用至量子狀態向量的作業。 更正式的說，如果每個容錯 $ \epsilon>$0 都存在 $G _1、\ldots G_N $，讓 $G _N \ldots G_1 $ 和 $U $ 之間的距離最多為 $ \epsilon $，這種閘道集就會是通用的。 理想的情況下，要達到 $ \epsilon $ 的這個距離所需的 $N $ 值應該使用 $ 1/\ epsilon $ 來調整 poly-logarithmically。
...
雖然先前的構成最常用的基本閘道，可描述堆疊邏輯層級的作業（請將邏輯層級視為配量演算法的層級），但在演算法層級上考慮較少的基本作業通常很方便，例如更接近函數描述層級的作業。 幸好，問 # 也有可用來執行較高層級 unitaries 的方法，而這可讓您在不明確分解所有專案到 Clifford 和 $T $-閘道的情況下，執行高階演算法。
