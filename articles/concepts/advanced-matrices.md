...
$ $ \begin{bmatrix}1 \\\\ 0 \\\\ 0 \end{bmatrix}、\begin{bmatrix}0 \\\\ 1 \\\\ 0 \ end {bmatrix} 和 \begin{bmatrix}0 \\\\ 0 \\\\1 \ 結束 {bmatrix} $ $
...
對於 Hermitian 和單一矩陣（基本上是在量子運算中唯一遇到的矩陣），有一個稱為[*spectral 定理*](https://en.wikipedia.org/wiki/Spectral_theorem)的一般結果，其判斷提示如下：針對任何 Hermitian 或單一矩陣 $M $，有一個單一 $U $，使其 $M = U ^ \dagger D U $ 用於某個對角線矩陣 $D $。 此外，$D $ 的對角線專案會是 $M $ 的特徵值。
