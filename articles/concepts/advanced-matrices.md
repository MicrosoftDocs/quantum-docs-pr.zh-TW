...
這一點很重要，因為配量機械時間演進是由 $ e ^ { IB } $ for Hermitian matrix $ B $ 形式的單一矩陣所描述。 基於這個理由，執行矩陣指數是量子運算的基本部分，因此會 Q# 提供用來描述這些作業的內建常式。
有許多方法可以在傳統電腦上計算矩陣指數，而在一般的數值將逼近中，這種指數是使用危險太。  請參閱[*Cleve Moler 和 Charles Van 貸款。「用來計算矩陣指數的十九可疑方法」。SIAM 計算期刊評論 20.4 (1978) ： 801-836*](https://doi.org/10.1137/S00361445024180) ，以取得相關挑戰的詳細資訊。
