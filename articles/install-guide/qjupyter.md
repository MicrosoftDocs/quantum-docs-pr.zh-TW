...
> [!NOTE]
> * 在 [Q # Jupyter 筆記本] 中，您只能執行 Q # 程式碼，而且無法從外部主機程式（例如 Python 或C#檔案）呼叫作業。 如果您的目標是要將外部傳統主機程式與配量程式合併，則此環境不適用。
...
1. 建立 `Hello World` 應用程式來驗證安裝
    ...
    - 在新的資料格中，使用 `%simulate` 命令來執行您剛才建立的作業（在模擬器中）：
        ...
        您應該會在畫面上看到列印的訊息，以及您叫用之作業的結果（在這裡，我們會看到空的元組 `()`，因為我們的作業只會傳回 `Unit` 類型）。
