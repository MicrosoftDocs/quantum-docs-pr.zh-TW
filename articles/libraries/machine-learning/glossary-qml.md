...
在 QML 程式庫中，修改超參數的最佳方式是藉由覆寫 UDT [`TrainingOptions`](xref:microsoft.quantum.machinelearning.trainingoptions)的預設值。 若要這麼做，我們使用函式[`DefaultTrainingOptions`](xref:microsoft.quantum.machinelearning.defaulttrainingoptions)來呼叫它，並套用運算子 `w/` 來覆寫預設值。 例如，若要使用100000測量和0.01 的學習率：
 ```qsharp
