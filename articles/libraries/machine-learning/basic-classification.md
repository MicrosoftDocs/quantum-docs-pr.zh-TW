...
- 驗證模型以判斷其正確性
    ...
    若要執行，您是來自 Python 的 Q # 分類器，請將下列程式碼儲存為 `host.py`。 請記住，您也需要問 # 檔案 `Training.qs`，本教學課程稍後會有說明。
    ...
    若要從C#執行，您是的 Q # 分類器，請將下列程式碼儲存為 `Host.cs`。 請記住，您也需要問 # 檔案 `Training.qs`，本教學課程稍後會有說明。
    ...
    若要從C# Visual Studio 執行新的 Q # 程式，請修改 `Host.cs` 以包含下列C#程式碼。 請記住，您也需要問 # 檔案 `Training.qs`，本教學課程稍後會有說明。
...
## <a name="q-classifier-code"></a>問\# 分類器程式碼

現在讓我們來看看如何在 Q # 中定義主機程式所叫用的作業。
我們會將下列程式碼儲存在名為 `Training.qs`的檔案中。
...
- `TrainHalfMoonModel() : TrainWineModel() : (Double[], Double)`：此作業是程式碼的核心部分，會定義定型。 在這裡，我們會從程式庫中包含的資料集載入範例，我們會設定定型的超參數和初始參數，並藉由呼叫包含在程式庫中 `TrainSequentialClassifier` 作業來開始定型。 它會輸出參數和判斷分類器的偏差。
