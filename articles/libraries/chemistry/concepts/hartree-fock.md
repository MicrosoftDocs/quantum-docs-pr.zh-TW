...
這是因為大部分分子驅使分子的房間溫度數量（例如，反應速率）都是由配量狀態間的免費能源差異所構成，這會描述反應路徑中步驟的開始和結束，以及房間溫度（如中繼）狀態通常是「基礎」狀態。
...
```
您也可以使用 `SpinOrbital` 索引來為 wave 函數編制索引，然後將這些索引轉換成整數，如下所示。
```csharp
...
Hartree-Fock 狀態也可以從 `FermionHamiltonian` 重建，如下所示。
```csharp
