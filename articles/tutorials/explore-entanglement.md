...
在本教學課程中，我們會示範如何撰寫 Q# 程式來操控和測量 qubits，並示範重迭和會任何牽連的效果。
...
您也可以遵循敘述，而不需要安裝 QDK、查看程式設計語言的總覽， Q# 以及量子運算的第一個概念。
...
> * 示範 Q# 在模擬器中執行程式的量子會任何牽連。 

## <a name="demonstrating-qubit-behavior-with-the-qdk"></a>使用 QDK 示範 qubit 行為

其中，古典位元會有單一二進位值，例如 0 或 1，而[量子位元](xref:microsoft.quantum.glossary#qubit)的狀態則可以是 0 和 1 的**疊加**。  就概念而言，qubit 的狀態可以視為抽象空間中的方向， (也稱為向量) 。  Qubit 狀態可以是任何可能的方向。 兩個**古典狀態**是兩個方向，分別代表 100% 測量到 0 的機率和 100% 測量到 1 的機率。
...
現在，我們已準備好示範如何 Q# 表示此行為。  您將從最簡單的程式著手建置，以示範量子疊加和量子纏結。
...
在此情況下，我們稱之為專案 `Bell` 。 這會產生兩個檔案： `Bell.csproj` 、專案檔和 `Program.qs` 應用程式的範本， Q# 我們將用它來撰寫應用程式。 的內容 `Program.qs` 應該是：
...
我們的目標是要以特定的量子狀態準備兩個 qubits，並示範如何在 qubits 上操作 Q# 以變更其狀態，並示範重迭和會任何牽連的效果。 我們會逐一建立這段程式，以引進 qubit 狀態、作業和測量。

### <a name="initialize-qubit-using-measurement"></a>使用測量來初始化 qubit

在下面的第一個程式碼中，我們會示範如何使用中的 qubits Q# 。  我們會介紹兩項作業， [`M`](xref:microsoft.quantum.intrinsic.m) 並 [`X`](xref:microsoft.quantum.intrinsic.x) 轉換 qubit 的狀態。 在此程式碼片段中定義的 `SetQubitState` 作業需要一個量子位元做為參數和另一個參數 `desired`，後者代表我們想要的量子位元狀態。  `SetQubitState` 作業使用 `M` 作業對量子位元執行測量。  在中 Q# ，qubit 量值一律會傳回 `Zero` 或 `One` 。  如果測量傳回的值不等於所需的值，就 `SetQubitState` 會「翻轉」 qubit; 也就是說，它會執行作業 `X` ，這會將 qubit 狀態變更為新狀態，其中測量傳回和的機率 `Zero` `One` 會反轉。 如此一來，一律會將 `SetQubitState` 目標 qubit 置於所需的狀態。
...
「作業」 Q# 是一種量子副程式。 也就是說，它是可呼叫的常式，其中包含對其他量子作業的呼叫。

作業的引數會在括弧內指定為元組。

作業的傳回類型指定於冒號後面。 在此案例中，`SetQubitState` 作業不會傳回任何項目，因此會標示為傳回 `Unit`。 這相當於 Q# `unit` F # 中的，其大致類似于 c # 中的， `void` 而空的元組 (`Tuple[()]` Python 中的) 。
...
```
請注意，我們已在之前新增一行， `return` 以在主控台中列印說明訊息，並使用函式 (`Message`) [microsoft 量子. message]
...
在這兩種情況下，都會由編譯器來推斷變數的類型。 Q#不需要變數的任何類型注釋。

#### <a name="about-using-statements-in-q"></a>關於 `using` Q 中的語句\#

`using`語句也是的特別 Q# 。 它會用來配置要用於程式碼區塊中的量子位元。 在中 Q# ，所有 qubits 都是動態配置和釋放，而不是固定在複雜演算法的整個存留期內的資源。 `using` 陳述式會在一開始配置一組量子位元，並在區塊結尾處釋出這些量子位元。

## <a name="execute-the-code-from-the-command-line"></a>從命令列執行程式碼

若要執行程式碼，我們必須指定在提供命令*時可呼叫執行的編譯器* `dotnet run` 。 這是在檔案中進行簡單的變更， Q# 方法是 `@EntryPoint()` `TestBellState` 在此情況下，直接在可呼叫：作業的前面加入一行。 完整的程式碼應該是：
...
現在讓我們看看如何 Q# 在重迭中表達 qubits 的方式。  前面說過，量子位元的狀態可以是 0 和 1 的疊加。  我們將使用 `Hadamard` 作業來達成此狀態。 如果量子位元是在其中一個古典狀態 (其測量會傳回一律 `Zero` 或一律 `One` )，則 `Hadamard` 或 `H` 作業會將量子位元置於一種「量子位元測量結果 50% 的時間會傳回 `Zero`、50% 的時間會傳回 `One`」的狀態。  概念上可以將量子位元想成介於 `Zero` 和 `One` 之間。  現在，當我們模擬 `TestBellState` 作業時，會看到在測量之後，結果傳回 `Zero` 和 `One` 的次數大致相同。  
...
本教學課程[Grover 的搜尋](xref:microsoft.quantum.quickstarts.search)會示範如何建立和執行 Grover 搜尋，這是其中一個最受歡迎的量子運算演算法，並提供 Q# 可用於解決量子運算之實際問題的程式範例。  

[開始使用量子開發工具組](xref:microsoft.quantum.welcome)建議更多學習和配量程式設計的方法 Q# 。
