---
uid: Microsoft.Quantum.Oracles
title: Oracle 命名空間
ms.date: 1/23/2021 12:00:00 AM
ms.topic: managed-reference
qsharp.kind: namespace
qsharp.name: Microsoft.Quantum.Oracles
qsharp.summary: This namespaces provides user-defined types to decorate various kinds of oracles by their intended use in quantum algorithms such as phase estimation and amplitude amplification.
ms.openlocfilehash: 5d3f99c514e74f7991d472f33c1f43eb0184e43d
ms.sourcegitcommit: 71605ea9cc630e84e7ef29027e1f0ea06299747e
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/26/2021
ms.locfileid: "98854497"
---
# <a name="microsoftquantumoracles-namespace"></a><span data-ttu-id="0ede4-102">Oracle 命名空間</span><span class="sxs-lookup"><span data-stu-id="0ede4-102">Microsoft.Quantum.Oracles namespace</span></span>

<span data-ttu-id="0ede4-103">此命名空間可提供使用者定義型別，以根據其在量子演算法中的預定用途（例如階段估計和振幅放大）來裝飾各種類型的 oracle。</span><span class="sxs-lookup"><span data-stu-id="0ede4-103">This namespaces provides user-defined types to decorate various kinds of oracles by their intended use in quantum algorithms such as phase estimation and amplitude amplification.</span></span>

